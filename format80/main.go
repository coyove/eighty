...
func (w *words_t) updateURL(o *FormatOptions) {
...
func (w *words_t) join(opt *FormatOptions) *bytes.Buffer {
	...
	for i := 0; i < len(words); i++ {
		...
		if word.ty == runeContinues {
			...
			buf.WriteString(" class=conj>&rarr;")
		}
			...
			if length+t.len > opt.width {
				...
				if nobrk {
					if len1 > 0 {
						...
						t2.len = len1
						line = append(line, t2, &word_t{ty: runeContinues, len: 1, value: []rune{'\\'}})
						appendReset()
						t.value = t.value[len1:]
						t.len = t.len - len1
					} else {
						line = append(line, &word_t{ty: runeContinues, len: 1, value: []rune{'\\'}})
						appendReset()
					}
					goto AGAIN
				}
